...
async function reseed(table) {
    ...
    await prisma_1.prisma.$executeRawUnsafe(`SELECT setval(pg_get_serial_sequence('"${table}"', 'id'), GREATEST((SELECT COALESCE(MAX(id), 0) FROM "${table}"), 1))`);
}
async function main() {
    const jsonPath = path_1.default.resolve(env_1.env.legacyDataJson);
    if (!fs_1.default.existsSync(jsonPath)) {
        throw new Error(`Legacy data file not found at ${jsonPath}. Set LEGACY_DATA_JSON in .env.`);
    }
    const store = JSON.parse(fs_1.default.readFileSync(jsonPath, 'utf8'));
    const arr = (k) => (Array.isArray(store[k]) ? store[k] : []);
    console.log(`Importing from ${jsonPath} ...`);
    // Members
    const members = arr('members');
    if (members.length) {
        await prisma_1.prisma.member.createMany({
            data: members.map((m) => ({
    ...
    if (statuses.length) {
        await prisma_1.prisma.projectStatus.createMany({
            data: statuses.map((s) => ({
    ...
    for (const [jsonKey, delegateName] of Object.entries(ITEM_TABLES)) {
        ...
        const delegate = prisma_1.prisma[delegateName];
    ...
    for (const m of members) {
        ...
        if (!email)
            continue;
        const temp = crypto_1.default.randomBytes(6).toString('base64url'); // ~8 chars
        const passwordHash = await bcryptjs_1.default.hash(temp, 10);
    ...
    if (creds.length) {
        ...
        for (const c of creds)
            console.log(`  ${c.email.padEnd(36)} ${c.tempPassword}`);
    }
    else {
...
//# sourceMappingURL=migrate-from-json.js.map
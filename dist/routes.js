...
//     }
//     if (type === 'qc') {
//       if ((method === 'POST' || method === 'DELETE') && !admin) return deny('Only admins can add or remove QA/QC entries')
//       return next()
//     }
//     if (type === 'task') {
...
function buildRouter() {
    ...
    // Single-round-trip payload for ProjectDetail's Dashboard tab. Replaces 10
    // separate /items/:type?projectId= calls (rfi, query, dispatch, wip, qc,
    // task, timesheet, standard, scope, input) with one request that fans the
    // same queries out in parallel on the server, avoiding client-side
    // connection-pool queuing. Cached the same way the other list endpoints are.
    r.get('/projects/:id/dashboard', (req, res) => {
        const projectId = int(req.params.id);
        const cacheKey = `projectDashboard:${projectId}`;
        const DASHBOARD_TYPES = ['rfi', 'query', 'dispatch', 'wip', 'qc', 'task', 'timesheet', 'standard', 'scope', 'input'];
        send(res, () => (0, redis_1.getCachedJson)(cacheKey, 30, async () => {
            const results = await Promise.all(DASHBOARD_TYPES.map((type) => itemService.getByProject(projectId, type)));
            return Object.fromEntries(DASHBOARD_TYPES.map((type, i) => [type, results[i]]));
        }));
    });
    r.post('/projects', projectGuard, (req, res) => {
        const { name, client, location, discipline, type, quoted_hours, start_date, end_date, client_id } = req.body;
    ...
    // ── Cross-cutting (reminders) ─────────────────────────────────────────────
    // Single-round-trip payload for reminders (badge count + panel list).
    // Replaces multiple client calls (/all/wip, /all/dispatches, /all/tasks,
    // /projects, /members, /all/timesheets) with one request that fans out on the server.
    r.get('/reminders/raw', (_req, res) => {
        const cacheKey = 'reminders:raw';
        send(res, () => (0, redis_1.getCachedJson)(cacheKey, 30, async () => {
            const [wip, dispatches, tasks, projects, members, timesheets] = await Promise.all([
                itemService.allOpenWip(),
                itemService.allDispatches(),
                itemService.allTasks(),
                projectService.getAll(),
                memberService.getAll(),
                itemService.allTimesheets(),
            ]);
            return { wip, dispatches, tasks, projects, members, timesheets };
        }));
    });

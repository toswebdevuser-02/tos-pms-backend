...
async function main() {
    const hash = await bcryptjs_1.default.hash(TEMP_PASSWORD, 10);
    ...
    for (const { name, email } of MEMBERS) {
        ...
        if (!member) {
            member = await prisma_1.prisma.member.create({ data: { name, email: lower, role: 'Employee' } });
        }
        // Create user only if one doesn't exist yet (check both email and memberId)
        const existingByEmail = await prisma_1.prisma.user.findUnique({ where: { email: lower } });
        const existingByMember = await prisma_1.prisma.user.findUnique({ where: { memberId: member.id } });
...
//# sourceMappingURL=seed-members.js.map